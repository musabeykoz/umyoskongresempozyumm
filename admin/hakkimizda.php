...
$page_title = 'Hakkımızda Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            ...
            $check_stmt = $db->query("SELECT id FROM umyos_hakkimizda LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                // Varsa güncelle
                $stmt = $db->prepare("UPDATE umyos_hakkimizda SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                // Yoksa ekle
                $stmt = $db->prepare("INSERT INTO umyos_hakkimizda (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
...
try {
    $stmt = $db->query("SELECT * FROM umyos_hakkimizda LIMIT 1");
    $hakkimizda = $stmt->fetch();
    if (!$hakkimizda) {
        $hakkimizda = ['icerik' => ''];
    }
} catch (PDOException $e) {
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-info-circle"></i> Hakkımızda Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Hakkımızda</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Hakkımızda İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($hakkimizda['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/hakkimizda.php" class="btn btn-secondary" target="_blank">
            ...
            <!-- Önizleme Bölümü -->
            <?php if (!empty($hakkimizda['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($hakkimizda['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($hakkimizda['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($hakkimizda['icerik'])));
                        }
