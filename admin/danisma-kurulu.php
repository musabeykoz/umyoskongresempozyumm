...
$page_title = 'Danışma Kurulu Yönetimi - ' . SITE_TITLE;
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        if (strlen(trim($icerik)) > 0) {
            try {
                ...
                $check_stmt = $db->query("SELECT id FROM umyos_danisma_kurulu LIMIT 1");
                $existing = $check_stmt->fetch();
                
                if ($existing) {
                    // Varsa güncelle
                    $stmt = $db->prepare("UPDATE umyos_danisma_kurulu SET icerik = ? WHERE id = ?");
                    $stmt->execute([$icerik, $existing['id']]);
                } else {
                    // Yoksa ekle
                    $stmt = $db->prepare("INSERT INTO umyos_danisma_kurulu (icerik) VALUES (?)");
                    $stmt->execute([$icerik]);
                }
                
                $success_message = 'Danışma Kurulu içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_danisma_kurulu LIMIT 1");
    $danisma_kurulu = $stmt->fetch();
    if (!$danisma_kurulu) {
        $danisma_kurulu = ['icerik' => ''];
    }
} catch (PDOException $e) {
    $danisma_kurulu = ['icerik' => ''];
}
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-user-friends"></i> Danışma Kurulu Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Danışma Kurulu</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Danışma Kurulu İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($danisma_kurulu['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/danisma-kurulu.php" class="btn btn-secondary" target="_blank">
            ...
            <!-- Önizleme Bölümü -->
            <?php if (!empty($danisma_kurulu['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($danisma_kurulu['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($danisma_kurulu['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($danisma_kurulu['icerik'])));
                        }
