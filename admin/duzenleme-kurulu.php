...
$page_title = 'Düzenleme Kurulu Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            ...
            $check_stmt = $db->query("SELECT id FROM umyos_duzenleme_kurulu LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                // Varsa güncelle
                $stmt = $db->prepare("UPDATE umyos_duzenleme_kurulu SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                // Yoksa ekle
                $stmt = $db->prepare("INSERT INTO umyos_duzenleme_kurulu (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
            
            $success_message = 'Düzenleme Kurulu içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_duzenleme_kurulu LIMIT 1");
    $duzenleme_kurulu = $stmt->fetch();
    if (!$duzenleme_kurulu) {
        $duzenleme_kurulu = ['icerik' => ''];
    }
} catch (PDOException $e) {
    $duzenleme_kurulu = ['icerik' => ''];
}
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-users-cog"></i> Düzenleme Kurulu Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Düzenleme Kurulu</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Düzenleme Kurulu İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($duzenleme_kurulu['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/duzenleme-kurulu.php" class="btn btn-secondary" target="_blank">
            ...
            <!-- Önizleme Bölümü -->
            <?php if (!empty($duzenleme_kurulu['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($duzenleme_kurulu['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($duzenleme_kurulu['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($duzenleme_kurulu['icerik'])));
                        }
