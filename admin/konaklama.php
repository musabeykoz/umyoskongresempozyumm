...
$page_title = 'Konaklama Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            $check_stmt = $db->query("SELECT id FROM umyos_konaklama LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                $stmt = $db->prepare("UPDATE umyos_konaklama SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                $stmt = $db->prepare("INSERT INTO umyos_konaklama (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
            
            $success_message = 'Konaklama içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_konaklama LIMIT 1");
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-hotel"></i> Konaklama Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Konaklama</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Konaklama İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($konaklama['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/konaklama.php" class="btn btn-secondary" target="_blank">
            ...
            <?php if (!empty($konaklama['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($konaklama['icerik']), $allowed_tags);
                        if (!preg_match('/<[^>]+>/', trim($konaklama['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($konaklama['icerik'])));
                        }
