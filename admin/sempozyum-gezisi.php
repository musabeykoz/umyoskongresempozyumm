...
$page_title = 'Sempozyum Gezisi Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            ...
            $check_stmt = $db->query("SELECT id FROM umyos_sempozyum_gezisi LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                // Varsa güncelle
                $stmt = $db->prepare("UPDATE umyos_sempozyum_gezisi SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                // Yoksa ekle
                $stmt = $db->prepare("INSERT INTO umyos_sempozyum_gezisi (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
            
            $success_message = 'Sempozyum Gezisi içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_sempozyum_gezisi LIMIT 1");
    $sempozyum_gezisi = $stmt->fetch();
    if (!$sempozyum_gezisi) {
        $sempozyum_gezisi = ['icerik' => ''];
    }
} catch (PDOException $e) {
    $sempozyum_gezisi = ['icerik' => ''];
}
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-map-marked-alt"></i> Sempozyum Gezisi Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Sempozyum Gezisi</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Sempozyum Gezisi İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($sempozyum_gezisi['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/sempozyum-gezisi.php" class="btn btn-secondary" target="_blank">
            ...
            <!-- Önizleme Bölümü -->
            <?php if (!empty($sempozyum_gezisi['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($sempozyum_gezisi['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($sempozyum_gezisi['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($sempozyum_gezisi['icerik'])));
                        }
