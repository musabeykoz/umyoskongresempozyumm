...
$page_title = 'Bilim Kurulu Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            ...
            $check_stmt = $db->query("SELECT id FROM umyos_bilim_kurulu LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                // Varsa güncelle
                $stmt = $db->prepare("UPDATE umyos_bilim_kurulu SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                // Yoksa ekle
                $stmt = $db->prepare("INSERT INTO umyos_bilim_kurulu (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
            
            $success_message = 'Bilim Kurulu içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_bilim_kurulu LIMIT 1");
    $bilim_kurulu = $stmt->fetch();
    if (!$bilim_kurulu) {
        $bilim_kurulu = ['icerik' => ''];
    }
} catch (PDOException $e) {
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-user-graduate"></i> Bilim Kurulu Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Bilim Kurulu</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Bilim Kurulu İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($bilim_kurulu['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/bilim-kurulu.php" class="btn btn-secondary" target="_blank">
            ...
            <!-- Önizleme Bölümü -->
            <?php if (!empty($bilim_kurulu['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($bilim_kurulu['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($bilim_kurulu['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($bilim_kurulu['icerik'])));
                        }
