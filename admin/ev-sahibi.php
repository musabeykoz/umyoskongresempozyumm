...
$page_title = 'Ev Sahibi Yönetimi - ' . SITE_TITLE;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($post_token)) {
        ...
        try {
            ...
            $check_stmt = $db->query("SELECT id FROM umyos_ev_sahibi LIMIT 1");
            $existing = $check_stmt->fetch();
            
            if ($existing) {
                // Varsa güncelle
                $stmt = $db->prepare("UPDATE umyos_ev_sahibi SET icerik = ? WHERE id = ?");
                $stmt->execute([$icerik, $existing['id']]);
            } else {
                // Yoksa ekle
                $stmt = $db->prepare("INSERT INTO umyos_ev_sahibi (icerik) VALUES (?)");
                $stmt->execute([$icerik]);
            }
            
            $success_message = 'Ev Sahibi içeriği başarıyla güncellendi.';
...
try {
    $stmt = $db->query("SELECT * FROM umyos_ev_sahibi LIMIT 1");
    ...
    <div class="admin-container">
        <div class="admin-header">
            <div class="admin-header-left">
                <h1><i class="fas fa-building"></i> Ev Sahibi Yönetimi</h1>
                <div class="admin-breadcrumb">
                    <a href="index.php"><i class="fas fa-home"></i> Yönetim Paneli</a>
                    <span class="separator">/</span>
                    <span class="current">Ev Sahibi</span>
                </div>
            </div>
        ...
        <div class="admin-content">
            ...
            <div class="form-section">
                <h2>
                    <i class="fas fa-edit"></i> Ev Sahibi İçeriğini Düzenle
                </h2>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
                    <div class="form-group">
                        <label>İçerik <span class="required">*</span></label>
                        <textarea id="icerik" name="icerik" required><?php echo htmlspecialchars($ev_sahibi['icerik']); ?></textarea>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <a href="<?php echo BASE_URL; ?>pages/ev-sahibi.php" class="btn btn-secondary" target="_blank">
            ...
            <?php if (!empty($ev_sahibi['icerik'])): ?>
                <div class="preview-section">
                    ...
                    <div class="preview-content">
                        ...
                        $preview_content = strip_tags(trim($ev_sahibi['icerik']), $allowed_tags);
                        // Eğer içerik HTML değilse (düz metin), satır sonlarını <br> ile değiştir
                        if (!preg_match('/<[^>]+>/', trim($ev_sahibi['icerik']))) {
                            $preview_content = nl2br(htmlspecialchars(trim($ev_sahibi['icerik'])));
                        }
