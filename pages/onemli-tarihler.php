...
$page_title = getPageTitle('Önemli Tarihler');
require_once('../includes/header.php');

// Önemli Tarihler içeriğini veritabanından çekme
try {
    $stmt = $db->query("SELECT icerik FROM umyos_onemli_tarihler LIMIT 1");
    $onemli_tarihler = $stmt->fetch();
    if (!$onemli_tarihler || empty($onemli_tarihler['icerik'])) {
        // Varsayılan içerik
        $icerik = 'Önemli Tarihler içeriği henüz eklenmemiş.';
    } else {
        $icerik = $onemli_tarihler['icerik'];
    }
        ...
        <div class="container full-width">
            <div class="content-area">
                <div class="page-content">
                    <h2 class="news-title">
                        <i class="fas fa-calendar-alt"></i> Önemli Tarihler
                    </h2>
