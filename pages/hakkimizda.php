...
$page_title = getPageTitle('Hakkımızda');
require_once('../includes/header.php');

// Hakkımızda içeriğini veritabanından çekme
try {
    $stmt = $db->query("SELECT icerik FROM umyos_hakkimizda LIMIT 1");
    $hakkimizda = $stmt->fetch();
    if (!$hakkimizda || empty($hakkimizda['icerik'])) {
        // Varsayılan içerik
        $icerik = 'Uluslararası Meslek Yüksekokulları Sempozyumu (UMYOS), meslek yüksekokullarının eğitim-öğretim kalitesinin artırılması, akademik gelişmelerin paylaşılması ve sektör-üniversite iş birliğinin güçlendirilmesi amacıyla düzenlenen önemli bir bilimsel platformdur.';
    } else {
        $icerik = $hakkimizda['icerik'];
    }
