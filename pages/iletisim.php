...
$page_title = getPageTitle('İletişim');
require_once('../includes/header.php');

// İletişim içeriğini veritabanından çekme
try {
    $stmt = $db->query("SELECT icerik FROM umyos_iletisim LIMIT 1");
    $iletisim = $stmt->fetch();
    if (!$iletisim || empty($iletisim['icerik'])) {
        // Varsayılan içerik
        $icerik = 'İletişim içeriği henüz eklenmemiş.';
    } else {
        $icerik = $iletisim['icerik'];
    }
